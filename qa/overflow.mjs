...
try {
  ...
  const W = Number(process.env.VP ?? 390);
  ...
  for (const p of paths) {
    ...
    const r = await page.evaluate((W) => {
      const wide = [...document.querySelectorAll('body *')].filter(e => e.getBoundingClientRect().right > W + 1 || e.scrollWidth > e.clientWidth + 2 && getComputedStyle(e).overflowX === 'visible');
    ...
    if (r.scrollWidth > W) { console.log('OVERFLOW', p, r.scrollWidth, r.items); process.exitCode = 2; }
  }
  console.log(`${paths.length} pages measured at ${W}px`);
  await browser.close();
} finally { server.kill(); }
