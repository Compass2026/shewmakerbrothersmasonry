...
- Hero video, 2026-09-08: `src/config/site.ts` carries `heroVideo`, off by default. When set, the clip loads only on wide screens with motion allowed and a connection that is not metered, playing over the hero photograph, which stays the poster. It is currently set to `hero-lake.mp4`, which Tom approved on 2026-09-08 knowing it is AI-generated footage carrying a visible generated-content watermark; that decision is recorded in `docs/placeholders.md` section G, because the brand board otherwise forbids AI-generated imagery.
