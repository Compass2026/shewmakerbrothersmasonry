...
| 2026-09-08 | "Never claim awards, certifications or association memberships"; the brand board's avoid list bars "second to none", and the copy linter blocks that phrase. | **Not overridden. Alternative shipped, pending Tom's word.** Tom asked for "something like best or Number 1" in the H1. A superiority claim is unverifiable here: the business has no reviews, no Google Business Profile (section E) and no awards (section D), so "best" or "#1" would be a bare assertion on a site whose whole register is checkable fact. | The H1 took the definite article instead: **"The Masonry Contractor at the Lake of the Ozarks"**, which claims primacy without asserting a rank, and keeps the money keyword intact. A credential stamp under the lead carries three claims that are already evidenced elsewhere on the site: Third generation · Our own crews · Footing to cap. If Tom confirms he wants the literal wording, it is one string in `src/pages/index.astro` (`h1`) and one line in `qa/lint-copy.mjs`. |
