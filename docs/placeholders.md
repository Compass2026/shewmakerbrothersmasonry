...
| --- | --- | --- | --- |
| 2026-09-08 | "Never use a stock or AI-generated image." | **Overridden by Tom.** He was shown the evidence that the first hero clip was AI-generated footage: a four-pointed generated-content sparkle burned into the corner, and a source filename that is a truncated text prompt. He answered "Go ahead and publish it." It was replaced the same evening with a second generation, from `docs/hero-video-brief.md`. | The clip plays behind the hero, at 1280x720 on desktop and a 1.05 MB rendition on phones. **The replacement carries no visible watermark**, checked on every corner across the clip, so nothing is cropped or painted out; Google's invisible SynthID marker is presumed still present and does not affect how it looks. Still AI footage, still an override. `src/config/site.ts` → `heroVideo` and `heroVideoSmall`. Remove by setting both to `null`. |
