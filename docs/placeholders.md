...
| --- | --- | --- | --- |
| 2026-09-08 | "Never use a stock or AI-generated image." | **Overridden by Tom.** He was shown the evidence that the first hero clip was AI-generated footage: a four-pointed generated-content sparkle burned into the corner, and a source filename that is a truncated text prompt. He answered "Go ahead and publish it." It was replaced the same evening, twice, with later generations from `docs/hero-video-brief.md`. The clip in place is the third: a low tracking shot along the wall face, chosen over an aerial of the same job because the stonework fills the frame rather than sitting small in a landscape. | The clip plays behind the hero, at 1280x720 on desktop and a 1.05 MB rendition on phones. **The clip in place carries no visible watermark**, checked on every corner across its length, so nothing is cropped or painted out; Google's invisible SynthID marker is presumed still present and does not affect how it looks. Still AI footage, still an override. `src/config/site.ts` → `heroVideo` and `heroVideoSmall`. Remove by setting both to `null`. |
...
| 2026-09-08 | WCAG AA text contrast on the hero, which `qa/hero-contrast.mjs` measures on the rendered pixels. | **Overridden by Tom, on phones first and then everywhere.** He asked for "no filter on the video, no dim, nothing" on mobile, saw it, and asked for the same on desktop. Every overlay is gone at every width: the text shade, the foot seat and the saturation filter. Measured against the bare footage the hero text falls to about 1:1 on the brightest frames, because the clip goes near-white behind the words. He was shown those frames before the desktop change. | No treatment over the footage anywhere. Legibility lives in the glyphs: a dark contour on all four sides plus a soft glow on the headline, lead, eyebrow and breadcrumbs, one pixel on phones and two on the much larger desktop display type. The ghost button gets a translucent seat behind it for the same reason. `qa/hero-contrast.mjs` reports these as `note` rather than `FAIL` and still prints the number, so a real regression stays visible. To restore the shade, put back the `.hero__inner::before` rule in `src/components/Hero.astro`. |
