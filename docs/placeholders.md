...
| 2026-09-08 | WCAG AA text contrast on the hero, which `qa/hero-contrast.mjs` measures on the rendered pixels. | **Overridden by Tom for phones only.** He asked for "no filter on the video, no dim, nothing" on mobile. Every overlay is gone there: the text shade, the foot seat and the saturation filter. Measured against the bare clip the hero text falls to about 1:1 on the brightest frames, because the footage goes near-white behind the words. He was shown those frames. | Phones get no treatment over the footage at all. Legibility moves into the glyphs: a one-pixel dark contour on all four sides plus a soft glow on the headline, lead and eyebrow, which leaves the video untouched. `qa/hero-contrast.mjs` reports these as `note` rather than `FAIL` below 700px and still prints the number, so a real regression is still visible. Desktop is unchanged and still passes on measurement: title 7.1:1, lead 6.6:1, eyebrow 5.5:1. To restore the phone shade, delete the `@media (max-width: 48rem)` block in `src/components/Hero.astro` that sets `display: none` on `.hero__inner::before`. |
